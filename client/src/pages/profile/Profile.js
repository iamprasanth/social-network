import { useState, useEffect } from "react";
import axios from "axios";
import api from "../../config/api";
import { useParams } from 'react-router-dom';
import Feeds from '../../components/feeds/Feeds';
import UserList from '../../components/users-list/UsersList';
import Moment from 'react-moment';
import Tabs from 'react-bootstrap/Tabs';
import Tab from 'react-bootstrap/Tab'
import "./profile.css";

export default function Profile({ history, props }) {

    const [posts, setPosts] = useState([]);
    const [user, setUser] = useState([]);
    var loggedInUser = JSON.parse(localStorage.getItem("user"));
    const [following, setFollowing] = useState(false);
    const params = useParams()

    useEffect(async () => {
        try {
            if (params.username == loggedInUser.username) {
                history.push('/myprofile')
            } else {
                const { data } = await axios.get(
                    api.getUserPosts + params.username,
                );
                console.log(loggedInUser)
                setFollowing(loggedInUser.followings.includes(data.data.user._id));
                setUser(data.data.user)
                setPosts(data.data.posts)
            }
        } catch (error) {
            console.log(error)
        }
    },
        [props]// Force complonent to reload when route parameter changes
    );

    const updateFollowingStatus = async () => {
        const config = {
            header: {
                "Content-Type": "application/json",
            },
        };
        const { data, error } = await axios.put(
            api.apiBaseUrl + 'users/' + user._id + '/follow',
            {
                receiving_user: user._id,
                requesting_user: loggedInUser._id
            },
            config
        );
        if (data.success) {
            loggedInUser = data.data.requestingUser;
            localStorage.setItem("user", JSON.stringify(loggedInUser))
            setUser(data.data.receivingUser)
            setFollowing(loggedInUser.followings.includes(user._id));
        }
    }

    return (
        <>
            <div className="feed">
                <div className="post">
                    <div className="post__top">
                        <img className="user__avatar post__avatar" src="data:image/png;base64," alt="" />
                        <div className="post__topInfo">
                            <h3>{user.username}
                                <span onClick={updateFollowingStatus} className={`material-icons cursor_pointer ${following ? "fill-blue" : ""}`}>
                                    person_add
                                </span></h3>
                            <p>
                                username: {user.username} <br />
                                Member since: <Moment format="DD MMM YYYY">{user.createdAt}</Moment> <br />
                                {user.bio}
                            </p>
                        </div>
                    </div>
                </div>
                <div className="post">
                    <Tabs defaultActiveKey="posts" id="uncontrolled-tab-example">
                        <Tab eventKey="posts" title="Posts">
                            <Feeds posts={posts} />
                        </Tab>
                        <Tab eventKey="followers" title="Followers">
                            <UserList users={user ? user.followers : {}} emptyMessage="No followers now" showFilter={true} />
                        </Tab>
                        <Tab eventKey="followings" title="Followings">
                            <UserList users={user ? user.followings : {}} emptyMessage="Following no one" showFilter={true} />
                        </Tab>
                    </Tabs>
                </div>
            </div>
        </>
    );
}
