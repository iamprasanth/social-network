...
export const Post = ({ post }) => {
    ...
    const showDeleteButton = (user._id == post.userId._id) ? true : false;
    const config = {
        header: {
    ...
    const [likesStates, setLikesStates] = useState({
        likes: post.likes,
        liked: post.likesBy.includes(user._id)
    });
    const { likes, liked } = likesStates;
        ...
        return (
            <>
                <div className="post">
                    ...
                    <div onClick={() => redirectToPost()} className="cursor_pointer">
                        <div className="post__bottom">
                            <p>{post.description}</p>
                        </div>
                        <div className="post__image">
                            <img src={process.env.REACT_APP_PUBLIC_FOLDER + '/posts/' + post.image} alt="" />
                        </div>
                    </div>
                    <div className="post__options">
                        <div className="post__option" onClick={() => updateLike(post._id)} >
                            <span className={`material-icons ${liked ? "fill-blue" : ""}`}> thumb_up </span>
                            &nbsp;{(likes > 0) && ('(' + likes + ')')}
                        </div>
                        <div className="post__option cursor_pointer" onClick={() => redirectToPost()}>
                            <span className="material-icons"> chat_bubble_outline </span>
                            &nbsp;{(post.comments.length > 0) && ('(' + post.comments.length + ')')}
                        </div>
                        {
                            showDeleteButton &&
                            <div className="post__option cursor_pointer fill-red" onClick={() => deletePost()}>
